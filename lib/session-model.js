    ...
    return new Promise(function(resolve) {
        ...
        if( !("session" in ____) ) {
            ...
            if( !fs.existsSync( path.normalize(path.join(____.dirSessionFile, '/session.json')) ) ) {
                gulp.task("copy-session.json", function () {
                        return gulp.src( path.join(__dirname, '../default-settings/session.json') )
        ...
        function parseScreenshotsList(dir) {
            ...
            (function recursion(dir) {
                var filesOrDir = fs.readdirSync(dir);
                var files = [];
                var dirs = [];

                //Сортируем
                filesOrDir.forEach(function(file) {
...
__.prototype.connect = function(socket) {
    ...
    /*********************************/
    /*Настройки*/
    ____.sinhroOneEvent('global.changeLang', socket);
    ____.sinhroOneEvent('global.floatPanels', socket);
    ____.sinhroOneEvent('global.floatPanelsAlign', socket);
    ...
    ____.sinhroOneEvent('local.showBottomSpace', socket);
    /*Айфрейм с вёрсткой*/
